...
def index():
    ...
    # Iterate Over Shares and Add Price and Value
    for share in shares:
        quotes = lookup(share["symbol"])
        share["symbol"] = quotes["symbol"]
        share["price"] = quotes["price"]
        share["value"] = share["price"] * share["total_shares"]
        Total_value += share["value"]
        Grand_total += share["value"]

    # Ensure Money is Represented with 2 Decimal Places
...
def buy():
    ...
    if request.method == "POST":
        ...
        elif not shares or not shares.isdigit() or float(shares) < 1:
        ...
        flash(f"You bought {shares} shares of {symbol.upper()} for {usd(total_cost)}")
        return redirect("/")
    else:
...
def history():
    ...
    transactions = db.execute(
        "SELECT * FROM transactions WHERE user_id = :user_id ORDER BY timestammp DESC", user_id=session["user_id"])
    ...
    if request.method == "POST":
        ...
        for stock in stocks:
            if stock["symbol"] == symbol:
                if int(stock["total_shares"]) < shares:
            ...
            if stock["symbol"] == symbol:
                ...
                # Notify User of Sale
                flash(f"You have sold {shares} shares of {symbol} for {usd(total_sale)}")
